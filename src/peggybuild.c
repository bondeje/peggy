...
/*
int register_symbols(PeggyParser * parser, lexre * lexre) {
    SymbolIterator siter;
    SymbolIterator_init(&siter, lexre);
    Symbol * sym = SymbolIterator_next(&siter);
    int nsym_added = 0;
    while (sym) {
        if (!parser->symbol_map._class->in(&parser->symbol_map, sym)) {
            nsym_added++;
            // copy the current symbol
            Symbol * symc = MemPoolManager_next(parser->sym_mgr);
            *symc = *sym;
            symc->sym = MemPoolManager_malloc(parser->str_mgr, sym->sym_len);
            memcpy((void *)symc->sym, sym->sym, (size_t)sym->sym_len * sizeof(char));
            // build an identifier for the symbol definition
            size_t buf_len = sizeof(char) * (parser->export.len + 6 + size_t_strlen(parser->symbol_map.fill));
            Symbol * name = MemPoolManager_next(parser->sym_mgr);
            name->sym = MemPoolManager_malloc(parser->str_mgr, buf_len);
            name->sym_len = snprintf((char *)name->sym, buf_len, "%.*s_sym_%zu", (int)parser->export.len, parser->export.str, parser->symbol_map.fill);
            // print definition
            fprintf(parser->source_file, "Symbol %.*s = ", name->sym_len, name->sym);
            Symbol_fprint(parser->source_file, symc);            
        }
        sym = SymbolIterator_next(&siter);
    }
    return nsym_added;
}
*/
