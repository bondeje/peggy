...
int are_match(struct avramT3 * av, const char * string, size_t const size,
    size_t const start) {
    ...
    while (cur <= size && (status = are_update(av, string, size, &cur)) >= REGEX_WAIT) {
        // do nothing, are_update will advance cursor
    }
...
static inline int av_push_buffer(struct avramT3 * av, const char * string, size_t const size, size_t const start) {
 ...
 */
int are_update(struct avramT3 * av, const char * string, size_t const size, size_t * cursor) {
